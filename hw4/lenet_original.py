...
class C3_layer(nn.Module):
    def __init__(self):
        ...
        self.conv3layers = nn.ModuleList([nn.Conv2d(3,1,kernel_size = 5) for i in range(6)])
        self.conv4layers = nn.ModuleList([nn.Conv2d(4,1,kernel_size = 5) for i in range(9)])
        self.conv6layer = nn.Conv2d(6,1,kernel_size = 5) 



    def forward(self, x):
        L = []
        for i,layer in enumerate(self.conv3layers):
            L.append(layer(x[:,self.ch_in_3[i],:,:]) )
        for i,layer in enumerate(self.conv4layers):
            L.append(layer(x[:,self.ch_in_4[i],:,:]) )
        L.append(self.conv6layer(x))
        return torch.cat(L, dim=1) #batch
